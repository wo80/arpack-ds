/* UTIL\smout.f -- translated by f2c (version 20230428). */
...
int smout_(a_int *lout, a_int *m, a_int *n, float *a, a_int *lda, a_int *idigit, char *ifmt)
{
    /* Initialized data */
        ...
        if (ndigit <= 4)
        {
            ...
            for (k1 = 1; k1 <= i__1; k1 += 5)
            {
                ...
                for (i = k1; i <= i__2; ++i)
                {
                    ...
                    do_fio(&c__1, (char *)&i, (ftnlen)sizeof(a_int));
                }
                e_wsfe();
                i__2 = *m;
                for (i = 1; i <= i__2; ++i)
