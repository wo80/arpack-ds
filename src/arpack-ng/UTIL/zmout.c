/* D:\projects\Fortran\arpack-ng-3.9.1-patched\UTIL\zmout.f -- translated by f2c (version 20230428).
...
int zmout_(integer *lout, integer *m, integer *n, doublecomplex *a, integer *lda, integer *idigit, char *ifmt, ftnlen ifmt_len)
{
    /* Initialized data */

    static char icol[1 * 3] = "C"
                              "o"
                              "l";
            ...
            for (k1 = 1; k1 <= i__1; k1 += 2)
            {
                ...
                for (i__ = 1; i__ <= i__2; ++i__)
                {
                    if (k1 != *n)
                    {
                        ...
                        for (j = k1; j <= i__3; ++j)
                        {
                            do_fio(&c__2, (char *)&a[i__ + j * a_dim1], (ftnlen)sizeof(doublereal));
                        }
                        e_wsfe();
                    }
                    else
        ...
        else
        {
            ...
            for (k1 = 1; k1 <= i__1; ++k1)
            {
                ...
                for (i__ = 1; i__ <= i__2; ++i__)
                {
                    ...
                    s_wsfe(&io___20);
                    do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
                    do_fio(&c__2, (char *)&a[i__ + k1 * a_dim1], (ftnlen)sizeof(doublereal));
                    e_wsfe();
                    /* L90: */
                }
                /* L100: */
