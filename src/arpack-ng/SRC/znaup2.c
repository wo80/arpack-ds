/* D:\projects\Fortran\arpack-ng-3.9.1-patched\SRC\znaup2.f -- translated by f2c (version 20230428).
    ...
    *np = np0;
    ...
    if (msglvl > 2)
    {
        ...
        ivout_(&debug_1.logfil, &c__3, kp, &debug_1.ndigit,
               ...
               ", NCONV are",
               (ftnlen)26);
        zvout_(&debug_1.logfil, &kplusp, &ritz[1], &debug_1.ndigit,
               "_naup2:"
    ...
    if (nconv >= nev0 || iter > *mxiter || *np == 0)
    {
        ...
        if (msglvl > 4)
        {
            /* Computing 2nd power */
            i__1 = kplusp;
            zvout_(&debug_1.logfil, &kplusp, &workl[i__1 * i__1 + 1], &debug_1.ndigit, "_naup2: Eigenvalues computed by _neigh:", (ftnlen)39);
            /* Computing 2nd power */
            i__1 = kplusp;
            zvout_(&debug_1.logfil, &kplusp, &workl[i__1 * i__1 + kplusp + 1], &debug_1.ndigit,
        ...
        if (msglvl > 1)
        {
            zvout_(&debug_1.logfil, &kplusp, &ritz[1], &debug_1.ndigit,
                   ...
                   (ftnlen)26);
            zvout_(&debug_1.logfil, &kplusp, &bounds[1], &debug_1.ndigit, "_naup2: Sorted ritz estimates.", (ftnlen)30);
        }
    ...
    if (msglvl > 0)
    {
        ...
        if (msglvl > 1)
        {
            ...
            ivout_(&debug_1.logfil, &c__2, kp, &debug_1.ndigit,
                   ...
                   " and NP are",
                   (ftnlen)22);
            zvout_(&debug_1.logfil, nev, &ritz[*np + 1], &debug_1.ndigit, "_naup2: \"wanted\" Ritz values ", (ftnlen)29);
            zvout_(&debug_1.logfil, nev, &bounds[*np + 1], &debug_1.ndigit, "_naup2: Ritz estimates of the \"wanted\" values ", (ftnlen)46);
        }
    }
...
L50:
    ...
    if (msglvl > 2)
    {
        ivout_(&debug_1.logfil, &c__1, np, &debug_1.ndigit,
               ...
               (ftnlen)38);
        zvout_(&debug_1.logfil, np, &ritz[1], &debug_1.ndigit,
               "_naup2: valu"
               "es of the shifts",
               (ftnlen)28);
        if (*ishift == 1)
        {
            zvout_(&debug_1.logfil, np, &bounds[1], &debug_1.ndigit,
                   "_naup2"
