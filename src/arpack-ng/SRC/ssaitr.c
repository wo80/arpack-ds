...
L1000:
    ...
    if (msglvl > 2)
    {
        ...
        svout_(&debug_1.logfil, &i_one, rnorm, &debug_1.ndigit,"_saitr: B-norm of the current residual =");
    }
...
L80:
    ...
    if (msglvl > 2)
    {
        xtemp[0] = wnorm;
        xtemp[1] = *rnorm;
        svout_(&debug_1.logfil, &i_two, xtemp, &debug_1.ndigit,"_saitr: re-orthonalization ; wnorm and rnorm are");
    }
...
L90:
    ...
    if (msglvl > 0 && iter > 0)
    {
        ...
        if (msglvl > 2)
        {
            xtemp[0] = *rnorm;
            xtemp[1] = rnorm1;
            svout_(&debug_1.logfil, &i_two, xtemp, &debug_1.ndigit,"_saitr: iterative refinement ; rnorm and rnorm1 are");
        }
    }
