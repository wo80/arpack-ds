/* D:\projects\Fortran\arpack-ng-3.9.1-patched\SRC\snaupd.f -- translated by f2c (version 20230428).
...
int snaupd_(integer *ido, char *bmat, integer *n, char *which, integer *nev, real *tol, real *resid, integer *ncv, real *v, integer *ldv, integer *iparam, integer *ipntr, real *workd, real *workl, integer *lworkl, integer *info, ftnlen bmat_len, ftnlen which_len)
{
    /* Format strings */
    ...
    extern int ivout_(integer *, integer *, integer *, integer *, char *, ftnlen);
    static integer ritzr;
    extern int svout_(integer *, integer *, real *, integer *, char *, ftnlen), snaup2_(integer *, char *, integer *, char *, integer *, integer *, real *, real *, integer *, integer *, integer *, integer *, real *, integer *, real *, integer *, real *, real *, real *, real *, integer *, real *, integer *, real *, integer *, ftnlen, ftnlen);
    ...
    snaup2_(ido, bmat, n, which, &nev0, &np, tol, &resid[1], &mode, &iupd, &ishift, &mxiter, &v[v_offset], ldv, &workl[ih], &ldh, &workl[ritzr], &workl[ritzi], &workl[bounds], &workl[iq], &ldq, &workl[iw], &ipntr[1], &workd[1], info, (ftnlen)1, (ftnlen)2);
    ...
    if (msglvl > 0)
    {
        ...
        ivout_(&debug_1.logfil, &c__1, &np, &debug_1.ndigit,
               ...
               (ftnlen)48);
        svout_(&debug_1.logfil, &np, &workl[ritzr], &debug_1.ndigit,
               "_naupd"
               ": Real part of the final Ritz values",
               (ftnlen)42);
        svout_(&debug_1.logfil, &np, &workl[ritzi], &debug_1.ndigit,
               "_naupd"
               ": Imaginary part of the final Ritz values",
               (ftnlen)47);
        svout_(&debug_1.logfil, &np, &workl[bounds], &debug_1.ndigit,
               "_naup"
