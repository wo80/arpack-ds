/* D:\projects\Fortran\arpack-ng-3.9.1-patched\SRC\snaitr.f -- translated by f2c (version 20230428).
...
L80:
    ...
    if (msglvl > 2)
    {
        ...
        svout_(&debug_1.logfil, &c__2, xtemp, &debug_1.ndigit,
               ...
               (ftnlen)47);
        svout_(&debug_1.logfil, &j, &h__[j * h_dim1 + 1], &debug_1.ndigit, "_naitr: j-th column of H", (ftnlen)24);
    }
    ...
    if (j > *k + *np)
    {
        ...
        *ido = 99;
        ...
        for (i__ = max(1, *k); i__ <= i__1; ++i__)
        {
            ...
            {
                i__2 = *k + *np;
                tst1 = slanhs_("1", &i__2, &h__[h_offset], ldh, &workd[*n + 1], (ftnlen)1);
            }
            /* Computing MAX */
            r__2 = ulp * tst1;
        ...
        if (msglvl > 2)
        {
            i__1 = *k + *np;
            i__2 = *k + *np;
            smout_(&debug_1.logfil, &i__1, &i__2, &h__[h_offset], ldh, &debug_1.ndigit,
