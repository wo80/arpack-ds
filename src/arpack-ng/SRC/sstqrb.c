/* SRC\sstqrb.f -- translated by f2c (version 20230428). */
...
int sstqrb_(a_int *n, float *d, float *e, float *z, float *work, a_int *info)
{
...
L10:
    ...
    if (l1 <= nm1)
    {
        ...
        for (m = l1; m <= i__1; ++m)
        {
            ...
            if (tst <= sqrt((r__1 = d[m], dabs(r__1))) * sqrt((r__2 = d[m + 1], dabs(r__2))) * eps)
            {
                e[m] = 0.f;
                goto L30;
            }
...
L30:
    ...
    i__1 = lend - l + 1;
    anorm = slanst_("i", &i__1, &d[l], &e[l], (ftnlen)1);
    iscale = 0;
    if (anorm == 0.f)
    {
        goto L10;
    ...
    L40:
        if (l != lend)
        {
            ...
            for (m = l; m <= i__1; ++m)
            {
                ...
                tst = r__2 * r__2;
                if (tst <= eps2 * (r__1 = d[m], dabs(r__1)) * (r__2 = d[m + 1], dabs(r__2)) + safmin)
                {
                    goto L60;
                }
                /* L50: */
