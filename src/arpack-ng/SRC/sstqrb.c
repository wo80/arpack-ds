/* D:\projects\Fortran\arpack-ng-3.9.1-patched\SRC\sstqrb.f -- translated by f2c (version 20230428).
...
L10:
    ...
    if (l1 <= nm1)
    {
        ...
        for (m = l1; m <= i__1; ++m)
        {
            ...
            if (tst <= sqrt((r__1 = d__[m], dabs(r__1))) * sqrt((r__2 = d__[m + 1], dabs(r__2))) * eps)
            {
                e[m] = 0.f;
                goto L30;
            }
    ...
    L40:
        if (l != lend)
        {
            ...
            for (m = l; m <= i__1; ++m)
            {
                ...
                tst = r__2 * r__2;
                if (tst <= eps2 * (r__1 = d__[m], dabs(r__1)) * (r__2 = d__[m + 1], dabs(r__2)) + safmin)
                {
                    goto L60;
                }
                /* L50: */
