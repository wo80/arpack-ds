/* SRC\snapps.f -- translated by f2c (version 20230428). */
...
int snapps_(a_int *n, a_int *kev, a_int *np, float *shiftr, float *shifti, float *v, a_int *ldv, float *h, a_int *ldh, float *resid, float *q, a_int *ldq, float *workl, float *workd)
{
    ...
    for (jj = 1; jj <= i__1; ++jj)
    {
        ...
        if (msglvl > 2)
        {
            ...
            svout_(&debug_1.logfil, &i_one, &sigmar, &debug_1.ndigit,"_napps: The float part of the shift ");
            svout_(&debug_1.logfil, &i_one, &sigmai, &debug_1.ndigit,"_napps: The imaginary part of the shift ");
        }
        ...
        for (i = istart; i <= i__2; ++i)
        {
            ...
            if ((r__1 = h[i + 1 + i * h_dim1], dabs(r__1)) <= dmax(r__2, smlnum))
            {
                if (msglvl > 0)
                {
                    ...
                    svout_(&debug_1.logfil, &i_one, &h[i + 1 + i * h_dim1], &debug_1.ndigit,"_napps: off diagonal element.");
                }
                iend = i;
                h[i + 1 + i * h_dim1] = 0.f;
                goto L40;
    ...
    if (msglvl > 1)
    {
        svout_(&debug_1.logfil, &i_one, &q[kplusp + *kev * q_dim1], &debug_1.ndigit, "_napps: sigmak = (e_{kev+p}^T*Q)*e_{kev}");
        svout_(&debug_1.logfil, &i_one, &h[*kev + 1 + *kev * h_dim1], &debug_1.ndigit, "_napps: betak = e_{kev+1}^T*H*e_{kev}");
