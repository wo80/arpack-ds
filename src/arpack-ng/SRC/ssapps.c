/* D:\projects\Fortran\arpack-ng-3.9.1-patched\SRC\ssapps.f -- translated by f2c (version 20230428).
...
int ssapps_(integer *n, integer *kev, integer *np, real *shift, real *v, integer *ldv, real *h__, integer *ldh, real *resid, real *q, integer *ldq, real *workd)
{
    ...
    L20:
        ...
        {
            big = (r__1 = h__[i__ + (h_dim1 << 1)], dabs(r__1)) + (r__2 = h__[i__ + 1 + (h_dim1 << 1)], dabs(r__2));
    ...
    if (msglvl > 1)
    {
        ...
        {
            i__1 = *kev - 1;
            svout_(&debug_1.logfil, &i__1, &h__[h_dim1 + 2], &debug_1.ndigit, "_sapps: updated sub diagonal of H for next iteration", (ftnlen)52);
        }
    }
