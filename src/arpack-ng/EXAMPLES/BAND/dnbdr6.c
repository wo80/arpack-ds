...
/*     %-----------------------------------------------------% */
    ...
    if (n > 1000) {
	s_wsle(&io___5);
	do_lio(&c__9, &c__1, " ERROR with _NBDR6: N is greater than MAXN ", (
		ftnlen)43);
	e_wsle();
	goto L9000;
    } else if (nev > 25) {
	s_wsle(&io___6);
	do_lio(&c__9, &c__1, " ERROR with _NBDR6: NEV is greater than MAXNEV "
		, (ftnlen)47);
	e_wsle();
	goto L9000;
    } else if (ncv > 50) {
	s_wsle(&io___7);
	do_lio(&c__9, &c__1, " ERROR with _NBDR6: NCV is greater than MAXNCV "
		, (ftnlen)47);
	e_wsle();
	goto L9000;
    }
...
/*              %--------------------% */
		...
		dgbmv_("Notranspose", &n, &n, &kl, &ku, &c_b101, &a[kl], &
			c__50, &v[j * 1000 - 1000], &c__1, &c_b15, ax, &c__1, 
			(ftnlen)11);
		dgbmv_("Notranspose", &n, &n, &kl, &ku, &c_b101, &m[kl], &
...
/* Main program alias */ int dnbdr6_ () { MAIN__ (); return 0; }
