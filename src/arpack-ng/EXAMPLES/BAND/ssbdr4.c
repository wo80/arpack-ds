/* D:\projects\Fortran\arpack-ng-3.9.1-patched\EXAMPLES\BAND\ssbdr4.f -- translated by f2c (version 20230428).
...
/* Main program */ int MAIN__(void)
{
    ...
    /* Local variables */
    real a[50000]	/* was [50][1000] */, d__[100]	/* was [50][2] */, 
	    h__;
    integer j;
    ...
    integer iparam[11];
    extern /* Subroutine */ int ssband_(logical *, char *, logical *, real *, 
	    real *, integer *, real *, integer *, real *, real *, integer *, 
	    real *, integer *, integer *, char *, char *, integer *, real *, 
...
/*     ... Call SSBAND with shift-invert mode to find eigenvalues LAMBDA */
...
/* \SCCS Information: @(#) */
/* FILE: sbdr4.F   SID: 2.5   DATE OF SID: 08/26/96   RELEASE: 2 */
...
/*     %--------------------------------------------------% */
    ...
    if (n > 1000) {
	s_wsle(&io___4);
	do_lio(&c__9, &c__1, " ERROR with _SBDR4: N is greater than MAXN ", (
		ftnlen)43);
	e_wsle();
	goto L9000;
    } else if (nev > 25) {
	s_wsle(&io___5);
	do_lio(&c__9, &c__1, " ERROR with _SBDR4: NEV is greater than MAXNEV "
		, (ftnlen)47);
	e_wsle();
	goto L9000;
    } else if (ncv > 50) {
	s_wsle(&io___6);
	do_lio(&c__9, &c__1, " ERROR with _SBDR4: NCV is greater than MAXNCV "
		, (ftnlen)47);
	e_wsle();
	goto L9000;
    }
...
/*     %-------------------------------------% */
    ...
    rvec = TRUE_;
    ssband_(&rvec, "A", select, d__, v, &c__1000, &sigma, &n, a, m, &c__50, 
	    rfac, &kl, &ku, which, bmat, &nev, &tol, resid, &ncv, v, &c__1000,
	     iparam, workd, workl, &lworkl, iwork, &info, (ftnlen)1, (ftnlen)
	    2, (ftnlen)1);
...
/*        %-----------------------------------% */
	...
	e_wsle();
	s_wsle(&io___39);
	do_lio(&c__9, &c__1, " _SBDR4 ", (ftnlen)8);
	e_wsle();
	s_wsle(&io___40);
	...
	e_wsle();
	s_wsle(&io___44);
	do_lio(&c__9, &c__1, " The number of Lanczos vectors generated", (
		ftnlen)40);
...
/*        %----------------------------% */
	...
	i__1 = nconv;
	for (j = 1; j <= i__1; ++j) {
	    sgbmv_("Notranspose", &n, &n, &kl, &ku, &c_b97, &a[kl], &c__50, &
		    v[j * 1000 - 1000], &c__1, &c_b15, ax, &c__1, (ftnlen)11);
	    sgbmv_("Notranspose", &n, &n, &kl, &ku, &c_b97, &m[kl], &c__50, &
		    v[j * 1000 - 1000], &c__1, &c_b15, mx, &c__1, (ftnlen)11);
	    r__1 = -d__[j - 1];
...
/* L90: */
	...
	smout_(&c__6, &nconv, &c__2, d__, &c__50, &c_n6, "Ritz values and re"
		"lative residuals", (ftnlen)34);
    } else {
...
/*        %-------------------------------------% */
	...
	e_wsle();
	s_wsle(&io___55);
	do_lio(&c__9, &c__1, " Check the documentation of _sband ", (ftnlen)
		35);
	e_wsle();
	s_wsle(&io___56);
	do_lio(&c__9, &c__1, " ", (ftnlen)1);
...
/* Main program alias */ int ssbdr4_ () { MAIN__ (); return 0; }
