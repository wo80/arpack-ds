/* D:\projects\Fortran\arpack-ng-3.9.1-patched\EXAMPLES\BAND\snbdr2.f -- translated by f2c (version 20230428).
...
/*     %-------------------------------------% */
	...
	i__2 = lo + nx - 1;
	for (j = lo + 1; j <= i__2; ++j) {
	    a[isub + (j + 1) * 50 - 51] = -1.f / h2 + rho / 2.f / h__;
	    a[isup + j * 50 - 51] = -1.f / h2 - rho / 2.f / h__;
/* L40: */
	}
/* L50: */
    }
...
/*              %--------------------% */
		...
		sgbmv_("Notranspose", &n, &n, &kl, &ku, &c_b100, &a[kl], &
...
/* Main program alias */ int snbdr2_ () { MAIN__ (); return 0; }
