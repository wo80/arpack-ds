/* D:\projects\Fortran\arpack-ng-3.9.1-patched\EXAMPLES\BAND\dnbdr2.f -- translated by f2c (version 20230428).
...
/*     %-------------------------------------% */
	...
	i__2 = lo + nx - 1;
	for (j = lo + 1; j <= i__2; ++j) {
	    a[isub + (j + 1) * 50 - 51] = -1. / h2 + rho / 2. / h__;
	    a[isup + j * 50 - 51] = -1. / h2 - rho / 2. / h__;
/* L40: */
	}
/* L50: */
    }
...
/*              %--------------------% */
		...
		dgbmv_("Notranspose", &n, &n, &kl, &ku, &c_b100, &a[kl], &
...
/*              %------------------------% */
		...
		d__1 = -d__[j - 1];
		daxpy_(&n, &d__1, &v[j * 1000 - 1000], &c__1, ax, &c__1);
		daxpy_(&n, &d__[j + 49], &v[(j + 1) * 1000 - 1000], &c__1, ax,
			 &c__1);
		d__[j + 99] = dnrm2_(&n, ax, &c__1);
		dgbmv_("Notranspose", &n, &n, &kl, &ku, &c_b100, &a[kl], &
...
/* Main program alias */ int dnbdr2_ () { MAIN__ (); return 0; }
