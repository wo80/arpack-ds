...
/* Main program */ int MAIN__(void)
{
    ...
    /* Local variables */
    float a[50000] /* was [50][1000] */, d__[150] /* was [50][3] */, h__;
    a_int i__, j;
    ...
    {
        s_wsle(&io___5);
        do_lio(&c__9, &c__1, " ERROR with _NBDR5: N is greater than MAXN ", (ftnlen)43);
        e_wsle();
        goto L9000;
    }
    else if (nev > 25)
    {
        s_wsle(&io___6);
        do_lio(&c__9, &c__1, " ERROR with _NBDR5: NEV is greater than MAXNEV ", (ftnlen)47);
        e_wsle();
        goto L9000;
    }
    else if (ncv > 50)
    {
        s_wsle(&io___7);
        do_lio(&c__9, &c__1, " ERROR with _NBDR5: NCV is greater than MAXNCV ", (ftnlen)47);
        e_wsle();
        goto L9000;
    }
    ...
    if (info == 0)
    {
        ...
        for (j = 1; j <= i__1; ++j)
        {
            ...
            else if (first)
            {
                ...
                r__1 = -d__[j - 1];
                saxpy_(&n, &r__1, &v[j * 1000 - 1000], &c__1, ax, &c__1);
                saxpy_(&n, &d__[j + 49], &v[(j + 1) * 1000 - 1000], &c__1, ax, &c__1);
                d__[j + 99] = snrm2_(&n, ax, &c__1);
...
/* Main program alias */ int snbdr5_()
{
    MAIN__();
    return 0;
}
