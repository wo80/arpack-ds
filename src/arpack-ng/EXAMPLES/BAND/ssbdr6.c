/* D:\projects\Fortran\arpack-ng-3.9.1-patched\EXAMPLES\BAND\ssbdr6.f -- translated by f2c (version 20230428).
...
/*     ... Call SSBAND with Cayley mode to find eigenvalues LAMBDA such that */
/*                          A*x = LAMBDA*M*x. */
...
/*     %--------------------------------------------------% */
    ...
    if (n > 1000) {
	s_wsle(&io___4);
	do_lio(&c__9, &c__1, " ERROR with _SBDR6: N is greater than MAXN ", (
		ftnlen)43);
	e_wsle();
	goto L9000;
    } else if (nev > 25) {
	s_wsle(&io___5);
	do_lio(&c__9, &c__1, " ERROR with _SBDR6: NEV is greater than MAXNEV "
		, (ftnlen)47);
	e_wsle();
	goto L9000;
    } else if (ncv > 50) {
	s_wsle(&io___6);
	do_lio(&c__9, &c__1, " ERROR with _SBDR6: NCV is greater than MAXNCV "
		, (ftnlen)47);
	e_wsle();
	goto L9000;
    }
...
/*     | iterations allowed.  Mode 5 of SSAUPD is used     | */
...
/* Main program alias */ int ssbdr6_ () { MAIN__ (); return 0; }
