 ...
 * \EndLib
 */
int dsband_(a_bool *rvec, char *howmny, a_bool *select, double *d, double *z, a_int *ldz, double *sigma, a_int *n, double *ab, double *mb, a_int *lda, double *rfac, a_int *kl, a_int *ku, char *which, char *bmat, a_int *nev, double *tol, double *resid, a_int *ncv, double *v, a_int *ldv, a_int *iparam, double *workd, double *workl, a_int *lworkl, a_int *iwork, a_int *info)
{
...
L90:
    ...
    dsaupd_(&ido, bmat, n, which, nev, tol, &resid[1], ncv, &v[v_offset], ldv, &iparam[1], ipntr, &workd[1], &workl[1], lworkl, info);
    ...
    if (ido == -1)
    {
        ...
        else if (type == 4)
        {
            ...
            dgbmv_("Notranspose", n, n, kl, ku, &c_b50, &mb[itop + mb_dim1], lda, &workd[ipntr[0]], &c__1, &c_b52, &workd[ipntr[1]], &c__1);
            dgbtrs_("Notranspose", n, kl, ku, &c__1, &rfac[rfac_offset], lda, &iwork[1], &workd[ipntr[1]], n, &ierr);
            if (ierr != 0)
            {
                printf(" \n");
        ...
        else if (type == 6)
        {
            ...
            {
                dcopy_(n, &workd[ipntr[0]], &c__1, &workd[ipntr[1]], &c__1);
                dgbmv_("Notranspose", n, n, kl, ku, &c_b50, &ab[itop + ab_dim1], lda, &workd[ipntr[0]], &c__1, sigma, &workd[ipntr[1]], &c__1);
            }
    ...
    else
    {
        ...
        else
        {
            ...
            if (iparam[5] > 0)
            {
                ...
                dseupd_(rvec, "A", &select[1], &d[1], &z[z_offset], ldz, sigma, bmat, n, which, nev, tol, &resid[1], ncv, &v[v_offset], ldv, &iparam[1], ipntr, &workd[1], &workl[1], lworkl, info);
