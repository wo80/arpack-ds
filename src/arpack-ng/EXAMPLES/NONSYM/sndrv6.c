/* D:\projects\Fortran\arpack-ng-3.9.1-patched\EXAMPLES\NONSYM\sndrv6.f -- translated by f2c (version 20230428).
...
L20:
    ...
    if (ido == -1)
    {
        ...
        cgttrs_("N", &n, &c__1, cdl, cdd, cdu, cdu2, ipiv, ctemp, &c__256, &ierr, (ftnlen)1);
        if (ierr != 0)
        {
            s_wsle(&io___38);
            do_lio(&c__9, &c__1, " ", (ftnlen)1);
        ...
        else
        {
            ...
            for (j = 1; j <= i__1; ++j)
            {
                ...
                else if (first)
                {
                    ...
                    d__[j - 1] = (numr * denr + numi * deni) / slapy2_(&denr, &deni);
                    d__[j + 24] = (numi * denr - numr * deni) / slapy2_(&denr, &deni);
                    first = FALSE_;
                }
                else
                {
...
/* Main program alias */ int sndrv6_()
{
    MAIN__();
    return 0;
}
